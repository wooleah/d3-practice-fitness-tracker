...
// d3 line path generator -> generators outputs d value of path
const line = d3
  .line()
  .x(function (d) {
    return x(new Date(d.date));
  }) // return a value that this x coordinate is based on
  .y(function (d) {
    return y(d.energySpent);
  });
...
const update = (data) => {
  ...
  // sort data based on data objects
  data = data.sort((a, b) => new Date(a.date) - new Date(b.date));

  // set scale domains(gets min/max of the dates)
  x.domain(d3.extent(data, (d) => new Date(d.date)));
  y.domain([0, d3.max(data, (d) => d.energySpent)]);

  // update path data -> in an array** when you're using path.data to draw a line
  path.data([data]).attr("fill", "none").attr("stroke", "#00bfa5").attr("stroke-width", 2).attr("d", line);

  // create circles for objects
  // it's not in the DOM in the beginning -> going to be the virtual DOM
  ...
  circles.attr("cx", (d) => x(new Date(d.date))).attr("cy", (d) => y(d.energySpent));
  ...
  graph
    ...
    .on("mouseover", (d, i, n) => {
      ...
      xDottedLine
        ...
        .attr("y1", graphHeight)
        .attr("y2", y(d.energySpent));
      // set y dotted line coords (x1, x2, y1, y2)
      yDottedLine
        .attr("x1", 0)
        .attr("x2", x(new Date(d.date)))
        .attr("y1", y(d.energySpent))
        .attr("y2", y(d.energySpent));
  ...
  // call axes(generate shapes for xaxis and place them inside axisgroup)
